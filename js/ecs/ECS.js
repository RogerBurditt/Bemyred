...
ecs.addEntityToArray = function(array, requirements, eid){
  var eIndex = ecs.db.map(function(e){return e.id;}).indexOf(eid);
  if(!ecs.containsEid(eid, array)){
    if(ecs.checkEntity(eid, requirements)){
      array.push(ecs.db[eIndex]);
    }
  }
}

ecs.removeEntityFromArray = function(array, requirements, eid){
  var arrIndex = array.map(function(e){return e.id;}).indexOf(eid);
  if(!ecs.checkEntity(eid, requirements)){
    array.splice(arrIndex, 1);
  }
}

ecs.breakEntity = function(eid){
  var eIndex = ecs.db.map(function(e){return e.id;}).indexOf(eid);
  for(var key in ecs.db[eIndex]) {
    ecs.db[eIndex][key] = null;
  }
  ecs.db[eIndex] = {};
  ecs.db.splice(eIndex, 1);
  ecs.componentRemoved(eid);
}

ecs.containsEid = function(eid, array) {
  for(var i in array){
    if(array[i].id == eid){
      return true;
    }
